...
5. **Phase 4: widgets と processes**
   ...
   - ページを構成するセクション (`Posts`, `Tags`, `Profile`, `Layout`) を widget 化。
   - Home ページは `pages/home/index.tsx` で `view-posts` プロセスから取得したデータを widgets に配布。
   - 投稿詳細ページも同様に `view-post-detail` を定義。
