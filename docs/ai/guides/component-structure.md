...
各コンポーネントは必ず PascalCase のディレクトリを持ち、その直下に以下の 4 ファイルを配置する。
...
- `ComponentName.stories.tsx`
  - Storybook によるビジュアルテストと UI カタログ。
  - CSF v3 形式で記述し、主要な UI 状態を表す Story を最低 1 つ以上定義する。
  - 実装の詳細は `docs/ai/guides/storybook-stories.md` および `docs/guides/storybook-stories.md` を参照すること。
...
- `ComponentName.stories.tsx` は必須ファイルとし、コンポーネント実装と同時に作成する。
  - Story の実装ガイドラインは `docs/ai/guides/storybook-stories.md` を参照すること。
...
- `Logo.stories.tsx` でデフォルト表示とサイズバリエーションの Story を定義。
- `index.ts` から `export { Logo } from './Logo'` として公開する。

## 実装時の手順

1. 新規コンポーネントを追加するときは、ディレクトリと 4 ファイル（`ComponentName.tsx`, `index.ts`, `ComponentName.test.tsx`, `ComponentName.stories.tsx`）をテンプレートとして先に作成する。
2. 既存コンポーネントを移行する際は、`git mv` を用いて履歴を保ったままリネームする。
3. Stories ファイルが存在しない既存コンポーネントには、`ComponentName.stories.tsx` を追加で作成する。
4. 依存先にある `import` 文を `@layer/.../ComponentName` 形式に更新し、ビルド／テストが通ることを確認する。
5. 変更後は `pnpm lint`, `pnpm typecheck`, `pnpm test` を実行し、結果を記録する。

## 保守ノート

- 2025-10-29: 全 UI ディレクトリを本ガイドに従う構成へ移行。以後の追加・改修時もこの構成を必須とする。
- 2025-11-23: `ComponentName.stories.tsx` を必須ファイルとして追加。Storybook によるビジュアルテストを標準化。
